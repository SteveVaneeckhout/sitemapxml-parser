...
async function fetchInitial(url, opts) {
    ...
    const childResults = await withConcurrency(parsed.sitemaps.map((s) => () => fetchAndResolve(s.loc, opts, 1, seen)), opts.concurrency);
...
async function fetchAndResolve(url, opts, depth, seen) {
    ...
    const results = await withConcurrency(parsed.sitemaps.map((s) => () => fetchAndResolve(s.loc, opts, depth + 1, seen)), opts.concurrency);
    return results.flat();
}
async function fetchAndDecode(url, opts) {
    ...
    while (true) {
        ...
        try {
            ...
            throw new FetchError(`Failed to fetch "${currentUrl}": ${String(err)}`, currentUrl, null, err);
        }
        const { status } = response;
        if (status >= 300 && status < 400) {
            if (redirects >= opts.maxRedirects) {
                throw new FetchError(`Too many redirects (>${opts.maxRedirects}) fetching "${url}"`, currentUrl, status);
            }
...
async function readBody(response, url, contentType, maxBytes) {
    ...
    try {
        while (true) {
            ...
            if (total > maxBytes) {
                throw new FetchError(`Response body exceeds maxSizeBytes (${maxBytes}) at "${url}"`, url, response.status);
            }
            chunks.push(value);
        }
    }
